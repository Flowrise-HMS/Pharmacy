        ...
        <div class="pos-cart-container shrink-0">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 pos-cart-sticky space-y-4 border border-gray-200 dark:border-gray-700">
                ...
                @if($lastInvoiceNumber)
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 flex items-center justify-between" x-data="{ copied: false }">
                        <div class="text-sm text-green-800 dark:text-green-200 min-w-0">
                            <span class="font-medium">{{ __('Last invoice') }}:</span>
                            <span class="font-mono font-bold select-all break-all">{{ $lastInvoiceNumber }}</span>
                        </div>
                        <button type="button"
                            x-on:click="navigator.clipboard.writeText('{{ $lastInvoiceNumber }}').then(() => { copied = true; setTimeout(() => copied = false, 2000); })"
                            class="text-xs text-green-700 dark:text-green-300 hover:text-green-900 dark:hover:text-green-100 bg-green-100 dark:bg-green-800/50 px-2 py-1 rounded transition shrink-0 ml-2">
