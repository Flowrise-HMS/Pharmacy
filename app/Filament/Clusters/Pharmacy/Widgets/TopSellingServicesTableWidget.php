...
class TopSellingServicesTableWidget extends BaseWidget
{
    ...
    {
        return $table
            ->heading(__('Top selling services'))
            ->records(fn (): array => $this->reportRows('top_selling_services', 'label'))
            ->columns([
                TextColumn::make('label')->label(__('Service')),
                TextColumn::make('quantity')->label(__('Qty sold'))->numeric(),
                CurrencyColumn::make('revenue')->label(__('Revenue')),
            ])
            ->paginated(false)
            ->emptyStateHeading(__('No service sales in range'));
    }
}
